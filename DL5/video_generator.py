...
config = {
    ...
    "image_size": 720,
    "video_path": "demo_videos/Tennis_0069.mov",
    "frame_rate": 24,
}
...
for i in range(frame_count):
    try:
        ...
        if out is None:
            video_save_path = config["video_path"].split(".")[0] + "_result.mp4"
            fourcc = cv2.VideoWriter_fourcc(*'mp4v')
            out = cv2.VideoWriter(video_save_path, fourcc, config["frame_rate"], (frame.shape[1], frame.shape[0]))
        # save frame
        out.write(frame)
